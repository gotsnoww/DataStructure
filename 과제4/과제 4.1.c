...
4.1-1의 코드를 보면, newNode를 추가하는 과정에서 마지막 노드를 찾는 과정에 반복문이 사용되므로 수행시간은 O(n)이라고 볼 수 있다.
그러나 이와 달리 4.1-2에서는 tail을 사용하여 마지막 노드까지 가는 방법이 아주 간단해졌다. 수행시간 또한 매우 짧아져 O(1)이다.
자료의 크기가 작을 경우 이 두 가지 방법에서의 실행시간은 큰 차이가 없겠지만, 자료의 크기가 방대해 질경우 실행시간의 차이가 엄청나게 날 수 있다.
